 ...
 *   1. the program catalog loads 18 samples (0 templates — templates were
 *      removed in v1.4.0)
...
check('catalog loads 18 samples, 0 templates', () => {
  const catalog = loadProgramCatalog(ROOT);
  assert.strictEqual(catalog.filter((p) => p.kind === 'sample').length, 18, 'sample count');
  assert.strictEqual(catalog.filter((p) => p.kind === 'template').length, 0, 'template count');
  assert.strictEqual(catalog.length, 18, 'total count');
});
...
check('filenames unique + sorted numbering 01..18', () => {
  const catalog = loadProgramCatalog(ROOT);
